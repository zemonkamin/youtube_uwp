...
public static class Config
{
    ...
    public static void LoadUserToken()
    {
        ...
        if (ApplicationData.Current.LocalSettings.Values.ContainsKey("yt_refresh_token"))
        {
            _usertoken = ApplicationData.Current.LocalSettings.Values["yt_refresh_token"] as string;
        ...
        else if (ApplicationData.Current.LocalSettings.Values.ContainsKey("AuthToken"))
        {
            _usertoken = ApplicationData.Current.LocalSettings.Values["AuthToken"] as string;
            System.Diagnostics.Debug.WriteLine($"Config: Loaded token from local settings (old key): {(_usertoken.Length > 20 ? _usertoken.Substring(0, 20) + "..." : "Empty")}");
        }
    }
        ...
        try
        {
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, "https://oauth2.googleapis.com/token"))
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine("[OAuth] Failed to refresh access token: " + response.StatusCode + " " + error);
                    return string.Empty;
                }
                ...
                if (root.ContainsKey("expires_in"))
                {
                    try
                    {
                        expiresIn = root.GetNamedNumber("expires_in");
                    }
                    catch
                    {
                        expiresIn = 3600.0;
    ...
    private static async Task<List<PlaylistItem>> GetMyPlaylistsViaInnertubeAsync(string accessToken, int maxResults)
    {
        ...
        using (var request = new HttpRequestMessage(HttpMethod.Post, url))
        {
            ...
            var json = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
            return ParsePlaylistCards(json, maxResults);
        }
    }
    ...
    public static async Task<List<PlaylistItem>> GetChannelPlaylistsAsync(string channelId, int maxResults)
    {
        ...
        try
        {
            ...
            var json = await PostInnertubeJsonAsync("browse", payload, string.Empty, "WEB", "2.20250101");
            if (string.IsNullOrWhiteSpace(json))
            {
                return result;
            }
    ...
    public static async Task<PlaylistDetails> GetPlaylistDetailsAsync(string playlistId, string refreshToken, int maxVideos)
    {
        ...
        if (details != null && !string.IsNullOrWhiteSpace(accessToken))
        {
            var dataApiDetails = await GetPlaylistMetadataViaDataApiAsync(playlistId, accessToken).ConfigureAwait(false);
        ...
        {
            var dataApiVideos = await GetPlaylistVideosViaDataApiAsync(playlistId, accessToken, maxVideos).ConfigureAwait(false);
    ...
    public static async Task<PlaylistDetails> GetPlaylistContinuationAsync(string refreshToken, string continuationToken, int maxVideos)
    {
        if (string.IsNullOrWhiteSpace(continuationToken))
        {
            return null;
        ...
        if (!string.IsNullOrWhiteSpace(accessToken))
        {
            json = await PostTvBrowseAsync(accessToken, null, null, continuationToken).ConfigureAwait(false);
        }
        ...
        if (string.IsNullOrWhiteSpace(json))
        {
            json = await PostAndroidBrowseAsync(accessToken, null, null, continuationToken).ConfigureAwait(false);
        }
        ...
        {
            var root = JsonValue.Parse(json);
            details.ContinuationToken = ExtractHistoryContinuationToken(root);
        }
        catch (Exception ex)
        {
    ...
    public static async Task<List<CommentItem>> GetCommentsAsync(string videoId, string continuationToken = null)
    {
        if (string.IsNullOrWhiteSpace(videoId))
        {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(continuationToken))
            {
                ...
                var nextPayload = "{\"context\":{\"client\":{\"clientName\":\"WEB\",\"clientVersion\":\"2.20250101\",\"hl\":\"" + Hl + "\",\"gl\":\"" + Gl + "\"}},\"videoId\":\"" + videoId + "\"}";
            ...
            var payload = "{\"context\":{\"client\":{\"clientName\":\"WEB\",\"clientVersion\":\"2.20250101\",\"hl\":\"" + Hl + "\",\"gl\":\"" + Gl + "\"}},\"continuation\":\"" + continuationToken + "\"}";
            var url = "https://www.youtube.com/youtubei/v1/next?key=" + InnertubeApiKey;
    ...
    private static string ExtractCommentsContinuationToken(string json)
    {
        try
        {
            ...
            // Путь: contents.twoColumnWatchNextResults.results.results.contents
    ...
    {
        var result = ParseVideoCards(json, maxCount);
        if (result.Count > 0 || string.IsNullOrWhiteSpace(json))
        {
            return result;
        }
    ...
    // Card thumbnail. mqdefault is only 320x180 and looks soft on a ~360px-wide card, so use
    // hqdefault (480x360) instead: it is the largest size YouTube guarantees for every video
    // (hq720/maxresdefault exist only for HD uploads and would 404 into empty cards).
    // hqdefault is 4:3 with letterbox bars, so the card must draw it with UniformToFill —
    // that crops the bars away and leaves an effective 480x270 16:9 image.
    // Deliberately not going bigger: these lists hold dozens of cards and this device is
    // memory-tight.
...
public sealed class VideoCardItem
{
